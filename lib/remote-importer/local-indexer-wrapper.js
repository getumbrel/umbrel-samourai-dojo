 ...
 * @param port {number}
 * @param host {string}
 * @param protocol {'tcp' | 'tls'}
 * @return {Promise<void>}
 */
const getActiveIndexer = (port, host, protocol) => {
    return new Promise((resolve, reject) => {
        const client = protocol === 'tls' ? tls.connect(port, host) : net.connect(port, host)
        ...
        client.on('error', async (err) => {
            if (err.code === 'ECONNREFUSED') {
                ...
                resolve(getActiveIndexer(port, host, protocol))
            } else {
                reject(err)
            }
        })

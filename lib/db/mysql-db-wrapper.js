...
class MySqlDbWrapper {
    ...
    ping() {
        ...
        for (let c of this.pool._freeConnections) {
            c.query('SELECT 1', (error) => {
                if (debug && error) {
                    Logger.error(error, 'Db Wrapper : ping() : Ping Error')
                }
            })
        }
    }
    ...
    async _query(query, retries) {
        ...
        return new Promise((resolve, reject) => {
            try {
                this.pool.query(query, null, async (error, result) => {
                    if (error) {
    ...
    async ensureAddressId(address) {
        ...
        const query2 = mysql.format(sqlQuery2, parameters2)
     ...
     * @returns {Promise<object>} returns a map of addresses to IDs: {[address]: 100}
     */
     ...
     * @param {string} address - bitcoin address
     * @returns {Promise<number>} returns the number of transactions for the address
     */
    ...
    async getTxsByAddrAndXpubs(addresses, xpubs, page, nbTxsPerPage) {
      ...
      LIMIT ?,?'
        ...
        let query = mysql.format(sqlQuery, parameters)
    ...
    /**
     * Gets the last block
     * @returns {Promise<object | null>} returns the last block
     */
